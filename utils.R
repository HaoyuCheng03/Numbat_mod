...
#' filtering, normalization and capping (returns a gene x cell matrix)
#' @param count_mat dgCMatrix Gene expression counts (gene x cell)
#' @param lambdas_ref matrix Reference expression profiles
#' @param gtf dataframe Transcript gtf
#' @return dataframe Log(x+1) transformed normalized expression values for single cells
#' @keywords internal
smooth_expression_parallel = function(count_mat, lambdas_ref, gtf, window = 101, ncores = 1, verbose = FALSE) {
    ...
    # a gene x cell matrix
    exp_mat_norm = log2(exp_mat * 1e6 + 1) - log2(lambdas_ref * 1e6 + 1)

    # capping
    cap = 3
    exp_mat_norm[exp_mat_norm > cap] = cap
    exp_mat_norm[exp_mat_norm < -cap] = -cap  # gene x cell

    # center by cell
    exp_mat_norm <- sweep(exp_mat_norm, 2, colMeans(exp_mat_norm, na.rm = TRUE), "-")

    # preallocate and smooth per cell along genes
    G <- nrow(exp_mat_norm) # of genes
    C <- ncol(exp_mat_norm) # of cells
    exp_mat_smooth <- matrix(NA_real_, nrow = G, ncol = C,
                             dimnames = dimnames(exp_mat_norm))

    idx <- seq_len(C)
    parts <- parallel::mclapply(
      idx,
      function(j) RcppRoll::roll_mean(exp_mat_norm[, j], n = window,
                                      align = "center", fill = NA_real_),
      mc.cores = ncores
    )
    for (j in idx) exp_mat_smooth[, j] <- parts[[j]]

    # assign back dimnames
    dimnames(exp_mat_smooth) <- dimnames(exp_mat_norm)

    # fill NA elements
    na_idx <- is.na(exp_mat_smooth)
    if (any(na_idx)) exp_mat_smooth[na_idx] <- as.numeric(exp_mat_norm[na_idx])

    # exp_mat_smooth is a gene x cell matrix
    return(exp_mat_smooth)
}
...
#' Get average reference expression profile based on single-cell ref choices
#' @param lambdas_ref matrix Reference expression profiles
...
# helper function for calc_phi_mle_lnpois for vectorize
calc_phi_mle_lnpois_idx <- function(Y_all, base_c, idx, sigma_c,
                                    lower = 0.1, upper = 10) {
  ...
  sig_vec <- rep.int(sigma_c, length(idx))
  x_seg   <- as.integer(Y_all[idx])
  mu_base <- base_c[idx]

  start = max(min(1, upper), lower)

  minuslogl <- function(phi) {
    -sum(hahmmr::dpoilog(x_seg, mu_base + log(phi), sig_vec, log = TRUE))
  }

  res = stats4::mle(minuslogl = minuslogl, start = start, lower = lower, upper = upper)

  return(res@coef)


  # f_neglog <- function(phi) {
  #   -sum(hahmmr::dpoilog(x_seg, mu_base + log(phi), sig_vec, log = TRUE))
  # }
  # optimize(f_neglog, interval = c(lower, upper))$minimum
}
...
#' @keywords internal
#' The terminal nodes are clusters passed to this function.
#' Example
#'        “0”
#'      /   \
#' “0.1”    “0.2”
...
compare_segs = function(segs_x, segs_y, gaps = hahmmr::gaps_hg38) {
...
evaluate_calls = function(cnvs_dna, cnvs_call, gaps = hahmmr::gaps_hg38) {
