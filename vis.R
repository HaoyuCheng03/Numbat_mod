...
#' @param named_clusters named vector Names = cell IDs, values = centroid labels (matching tree tip labels)
...
#' @return ggplot panel
#' @export
plot_phylo_heatmap_centroids <- function(
  gtree,
  named_clusters,
  joint_post,
  segs_consensus,
  ...
  # --- small utilities -------------------------------------------------------
  .is_tbl <- function(x) inherits(x, "tbl_graph")
  .attach_counts <- function(gt, named_clusters) {
    if (!.is_tbl(gt)) return(gt)
    nd <- tidygraph::activate(gt, "nodes")
    if (!"n_cells" %in% colnames(as.data.frame(nd))) {
      counts <- as.data.frame(table(named_clusters), stringsAsFactors = FALSE)
      colnames(counts) <- c("name", "n_cells")
      gt <- nd %>% dplyr::left_join(counts, by = "name") %>%
        dplyr::mutate(n_cells = dplyr::coalesce(.data$n_cells, 0L)) %>%
        tidygraph::as_tbl_graph()
    }
    gt
  }
  .mark_lineage_auto <- function(gt) {
    if (!.is_tbl(gt)) return(gt)
    nd <- tidygraph::activate(gt, "nodes") %>% as.data.frame()
    if ("n_cells" %in% colnames(nd)) {
      mark_tumor_lineage_centroids(gt)
    } else {
      mark_tumor_lineage(gt)
    }
  }

  # --- sanitize inputs -------------------------------------------------------
  joint_post <- joint_post %>%
    dplyr::mutate(CHROM = as.integer(as.character(.data$CHROM)))
  segs_consensus <- segs_consensus %>%
    dplyr::mutate(CHROM = as.integer(as.character(.data$CHROM)))

  # handle multi-allelic vs simple
  if (!"n_states" %in% colnames(joint_post)) {
    joint_post <- joint_post %>%
      dplyr::mutate(
        n_states = ifelse(.data$cnv_state == "neu", 0, 1),
        cnv_states = .data$cnv_state
      )
  } else {
    joint_post <- joint_post %>%
      dplyr::group_by(.data$cell, .data$CHROM, .data$seg_end, .data$seg_start) %>%
      dplyr::mutate(p_cnv = sum(.data$p_cnv)) %>%
      dplyr::ungroup() %>%
      dplyr::distinct(.data$cell, .data$CHROM, .data$seg_end, .data$seg_start, .keep_all = TRUE) %>%
      dplyr::mutate(cnv_state = ifelse(.data$n_states > 1, .data$cnv_state_map, .data$cnv_state))
  }

  # --- tree panel (centroid-level) ------------------------------------------
  if (.is_tbl(gtree)) {
    gtree <- .attach_counts(gtree, named_clusters) %>%
      tidygraph::activate("edges") %>%
      dplyr::mutate(length = ifelse(.data$leaf, tip_length, .data$length))
  ...
  # centroid tip order
  centroid_order <- get_ordered_tips(tree_obj)

  # --- expand centroid order to cell order ----------------------------------
  # named_clusters: names(cell) -> centroid
  # build per-centroid list of cells preserving any stable order (alphabetical fallback)
  cent2cells <- split(names(named_clusters), as.character(named_clusters))
  cent2cells <- lapply(cent2cells, function(v) v[order(v)])  # deterministic

  # cell_order is centroid_order expanded by their member cells
  cell_order <- unlist(cent2cells[centroid_order], use.names = FALSE)
  cell_order <- cell_order[!is.na(cell_order)]  # safety

  # --- tree plotting panel ---------------------------------------------------
  if (show_phylo) {
    p_tree <- tryCatch({
      p <- ggtree::ggtree(tree_obj, ladderize = FALSE, size = branch_width) +
        ggplot2::theme(
          plot.margin = ggplot2::margin(0, 1, 0, 0, unit = "mm"),
      ...
      if (root_edge) p <- p + ggtree::geom_rootedge(size = branch_width)
      p
    }, error = function(e) {
      message("Phylogeny plotting failed: ", conditionMessage(e))
      ggplot2::ggplot() + ggplot2::theme_void()
    })
  } else {
    p_tree <- ggplot2::ggplot() + ggplot2::theme_void()
  }

  # --- index cells by expanded order ----------------------------------------
  joint_post <- joint_post %>%
    dplyr::filter(.data$cell %in% cell_order) %>%
    dplyr::mutate(cell = factor(.data$cell, levels = cell_order)) %>%
    dplyr::mutate(cell_index = as.integer(droplevels(.data$cell)))

  chrom_labeller <- function(chr) { chr[chr %in% c(19, 21, 22)] <- ""; chr }

  # --- CNV heatmap (cell-level) ---------------------------------------------
  p_segs <- ggplot2::ggplot(
    joint_post %>% dplyr::mutate(cnv_state = ifelse(.data$cnv_state == "neu", NA, .data$cnv_state))
  ) +
    ggplot2::geom_blank() +
    ggplot2::theme_classic() +
    ggplot2::geom_segment(
      ggplot2::aes(x = .data$seg_start, xend = .data$seg_end,
                   y = .data$cell_index, yend = .data$cell_index,
                   color = .data$cnv_state, alpha = .data$p_cnv),
      linewidth = line_width
    ) +
    ggplot2::geom_segment(
      inherit.aes = FALSE,
      ggplot2::aes(x = seg_start, xend = seg_end, y = 1, yend = 1),
      data = segs_consensus, linewidth = 0, color = "white", alpha = 0
    ) +
    ggplot2::theme(
      panel.spacing = grid::unit(0, "mm"),
      panel.border  = ggplot2::element_rect(size = 0.5, color = "gray", fill = NA),
    ...
    ggplot2::facet_grid(. ~ CHROM, space = "free", scales = "free",
                        labeller = ggplot2::labeller(CHROM = chrom_labeller)) +
    ggplot2::scale_alpha_continuous(range = c(0, 1), limits = c(p_min, 1), oob = scales::squish) +
    ggplot2::guides(alpha = "none") +
    ggplot2::xlab("Genomic position")

  # color map (with safe fallback for bamp)
  bamp_col <- if (exists("cnv_colors", inherits = TRUE) && !is.null(cnv_colors[["bamp"]])) cnv_colors[["bamp"]] else "#8B1A1A"
  p_segs <- p_segs +
    ggplot2::scale_color_manual(
      values = c(amp = "darkred", del = "darkblue", bamp = bamp_col, loh = "darkgreen", bdel = "blue"),
      labels = c(amp = "AMP", del = "DEL", bamp = "BAMP", loh = "CNLoH", bdel = "BDEL"),
      na.translate = FALSE
    )

  # --- tumor/normal divider (centroid → cells) -------------------------------
  if (tvn_line && .is_tbl(gtree)) {
    gtree_marked <- .mark_lineage_auto(gtree)

    tumor_centroids <- gtree_marked %>%
      tidygraph::activate("nodes") %>% dplyr::filter(.data$leaf) %>%
      as.data.frame() %>%
      dplyr::filter(.data$compartment == "tumor") %>%
      dplyr::pull(.data$name)

    tumor_cells <- names(named_clusters)[named_clusters %in% tumor_centroids]
    first_tumor_index <- which(cell_order %in% tumor_cells)[1]
    if (length(first_tumor_index) && !is.na(first_tumor_index)) {
      p_segs <- p_segs + ggplot2::geom_hline(yintercept = first_tumor_index,
                                             color = "royalblue", linewidth = 0.5, linetype = "dashed")
    }
  }

  # --- clone bar (cell-level) -----------------------------------------------
  if (clone_bar) {
    # per-cell clone labels
    if (is.null(clone_dict)) {
      # derive per-centroid clone, then map to cells
      if (.is_tbl(gtree) && "clone" %in% colnames(as.data.frame(tidygraph::activate(gtree, "nodes")))) {
        centroids_df <- tidygraph::activate(gtree, "nodes") %>% as.data.frame() %>% dplyr::filter(.data$leaf)
        cent_clone <- stats::setNames(centroids_df$clone, centroids_df$name)
        per_cell_clone <- cent_clone[as.character(named_clusters)]
        clone_dict <- factor(per_cell_clone, exclude = NULL)
        names(clone_dict) <- names(named_clusters)
      } else {
        # fallback: everything normal (clone 1)
        clone_dict <- factor(rep(1, length(cell_order)), levels = 1)
        names(clone_dict) <- cell_order
      }
      normal_clones <- if (.is_tbl(gtree)) {
        tidygraph::activate(gtree, "nodes") %>% as.data.frame() %>%
          dplyr::filter(.data$leaf, .data$compartment == "normal") %>%
          dplyr::pull(.data$clone) %>% unique()
      } else character(0)
    } else {
      normal_clones <- character(0)
    }
    ...
    if (is.null(pal_clone)) {
      ...
      nlev <- max(as.integer(levels(clone_dict)), na.rm = TRUE)
      colv <- getPalette(max(2, nlev))
      pal_clone <- stats::setNames(colv, as.character(seq_along(colv)))
      pal_clone[c(as.character(normal_clones), "1")] <- "gray"
    }
    ...
    if (!is.null(clone_post) && clone_stack) {
      ...
      p_clone <- data.frame(
        ...
        annot_bar(
          ...
          legend_title = clone_title, label_size = bar_label_size, raster = raster
        )
    }
  }

  # --- optional clone separators (cell-level) --------------------------------
  if (clone_line && !is.null(clone_dict)) {
    cd <- data.frame(cell = names(clone_dict), clone = as.integer(clone_dict), stringsAsFactors = FALSE)
    idx <- tapply(cd$cell, cd$clone, function(v) which(cell_order %in% v)[1])
    idx <- idx[!is.na(idx) & names(idx) != "1"]
    if (length(idx)) {
      p_segs <- p_segs + ggplot2::geom_hline(yintercept = unname(idx),
                                             color = "darkslategray", linewidth = 0.5, linetype = "dashed")
    }
  }
  ...
  if (!is.null(annot) && is.data.frame(annot)) {
    p_annot <- annot %>%
      ...
      lapply(function(ann) {
        annot_bar(
          ann, transpose = TRUE, pal_annot = pal_annot,
          legend_title = unique(ann$variable),
          label_size = bar_label_size, annot_scale = annot_scale, raster = raster
        )
      }) %>%
      patchwork::wrap_plots()

    if (clone_bar) {
      out <- (p_tree | p_clone | p_annot | p_segs) +
        patchwork::plot_layout(widths = c(tree_height, clone_bar_width, annot_bar_width, 15),
                               guides = "collect")
    } else {
      out <- (p_tree | p_annot | p_segs) +
        patchwork::plot_layout(widths = c(tree_height, annot_bar_width, 15),
                               guides = "collect")
    }
  } else if (clone_bar) {
    out <- (p_tree | p_clone | p_segs) +
      patchwork::plot_layout(widths = c(tree_height, clone_bar_width, 15),
                             guides = "collect")
  } else {
    out <- (p_tree | p_segs) +
      patchwork::plot_layout(widths = c(tree_height, 15), guides = "collect")
  }
...
#' Plot single-cell smoothed expression magnitude heatmap based on hclust from c
#'troids
...
#' @param named_clusters name vector cell names and the cluster they belong to
...
#' @export
plot_exp_roll_kmeans <- function(gexp_roll_wide, hc, gtf, named_clusters, lim = 0.8,
                          n_sample = 300, plot_tree = TRUE) {
  ...
  # proportional subsampling samples for plotting in each cluster
  cl_order <- hc$labels[order.hclust(hc)]
  cl_of_cell <- factor(as.character(named_clusters[cells]), levels = cl_order)
  cl_split   <- split(cells, cl_of_cell, drop = FALSE)

  sizes <- lengths(cl_split)
  total <- sum(sizes)


  if (n_sample >= total) {
    cell_blocks <- cl_split
  } else {
    take <- pmin(sizes, floor(sizes / total * n_sample))
    cell_blocks <- Map(sample, cl_split, take)
  }

  cell_order <- unlist(cell_blocks, use.names = FALSE)


  # dendrogram on the left (cluster-level tree)
  p_tree = ggtree::ggtree(hc, size = 0.2)

  # heatmap of random selected samples
  p_heatmap = gexp_norm_long %>%
    filter(cell %in% cell_order) %>%
    mutate(cell = factor(cell, levels = cell_order)) %>%
    mutate(cluster = as.character(named_clusters[as.character(cell)])) %>%
    mutate(cluster = factor(cluster, levels = rev(cl_order))) %>%
...
cnv_heatmap = function(segs, var = 'group', label_group = TRUE, legend = TRUE, exclude_gap = TRUE, genome = 'hg38') {
  ...
  if (genome == 'hg38') {
    ...
    gaps = hahmmr::gaps_hg38
  } else if (genome == 'hg19') {
    chrom_sizes = hahmmr::chrom_sizes_hg19
    gaps = hahmmr::gaps_hg19
  } else {
