...
log_mem = function() {
    m = pryr::mem_used()
    msg = paste0('Mem snapshot: ', signif(m/1e9, 3), 'Gb')
    log_message(msg)
}

#’ @keywords internal
#’ Log a snapshot of R’s CPU & elapsed time
log_time <- function(verbose = TRUE) {
  times <- proc.time()
  msg <- sprintf(
    "Time used: elapsed=%.2fs", times["elapsed"]
  )
  # use the same logger you use for log_mem()
  log_message(msg, verbose = verbose)
}
